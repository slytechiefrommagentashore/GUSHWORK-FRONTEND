...
// Sticky Header & Price Bar Show/Hide on Scroll
...
function handleScroll() {
  if (!scrollTicking) {
    window.requestAnimationFrame(() => {
      // Check if user has scrolled past the first fold (100vh)
      if (window.scrollY > window.innerHeight) {
        stickyHeader.classList.add("visible");
      } else {
        // Disappears when scrolling back up to the top fold
...
document.addEventListener("DOMContentLoaded", function () {
  const stickyPriceBar = document.getElementById("sticky-price-bar");
  ...
  if (stickyPriceBar) {
    window.addEventListener("scroll", () => {
      ...
      if (isMobile) {
        // Show the price bar after scrolling down roughly the height of the hero section
        // Fallback to 600px if heroSection isn't calculated properly
        const triggerHeight = heroSection ? heroSection.offsetHeight - 100 : 600; 
        
        if (window.scrollY > triggerHeight) {
          stickyPriceBar.classList.add("visible");
        } else {
          stickyPriceBar.classList.remove("visible");
        }
        
      } else {
        // Show the price bar ONLY at the Advanced HDPE Process section
        if (processSection) {
